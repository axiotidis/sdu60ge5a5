...
def readFirebase(Path, Key):
    try:
        fbData = fBaseDb.child(Path).child(Key).get()
        #print(fbData.val())
        return fbData.val()

    except:
        print("readFirebase function Error")
...
def setFirebase(Path, Key, Data):
    try:
        ...
        print("setFirebase function set Error")
...
def clearMonth():
    ...
    fbDest = "malus"
    updateFirebase(fbPath, fbDest, "0")
...
def caclBonus(monthId):
    db = pymysql.connect(address, username, password, database)
    curs=db.cursor()

    try:
        sql_select_query = "SELECT * FROM totalMonthly"
        curs.execute (sql_select_query)
        temp = curs.fetchone()
        c1 = float(temp[monthId])+1.0      #current month consumption in Wh
        c2 = float(temp[monthId-1])    #previus month consumption in Wh
        bonus = (c2/c1)*100              #calculate bonus
        curs.close()
        db.close()
        return int(bonus)
        ...
        print("Database calcBonus function read Error")
        curs.close()
        db.close()
...
def checkLights(date, time):
    ...
    readPath = "/users/"+cuser+"/profile/"
    readKey = "malus"
    #print("readPath= "+readPath)
    temp = readFirebase(readPath, readKey)   #read stored vaue
    #print("stored malus= "+str(temp))
    tempData = int(temp)+10
    fbData = str(tempData)
    #print("updated malus= "+fbData)
    fbPath = "/users/"+cuser+"/profile/"
    fbDest = "malus"
    updateFirebase(fbPath, fbDest, fbData)
...
while True:
    ...
    if isFireTime < time2upload:
        ...
        #calculate bonus
        myBonus = caclBonus(cmonthId)

        #update bonus in firebase
        fbPath = "/users/"+cuser+"/profile/"
        fbDest = "bonus"
        updateFirebase(fbPath, fbDest, str(myBonus))

        malPath = "/users/"+cuser+"/profile/"
        malKey = "malus"
        myMalus = readFirebase(malPath, malKey)
        #print("myBonus = "+str(myBonus))
        #print("myMalus = "+str(myMalus))
        
        myPoints = int(myBonus)-int(myMalus)
        #print("myPoints = "+str(myPoints))
        
        #update points in firebase
        fbPath = "/users/"+cuser+"/profile/"
        fbDest = "points"
        updateFirebase(fbPath, fbDest, str(myPoints))
